...
export const MobileAdvancedFilters = ({ onFiltersChange, isOpen, onToggle }: MobileAdvancedFiltersProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background">
      {/* Header */}
      <div className="sticky top-0 bg-background/95 backdrop-blur-md border-b border-border">
      ...
      {/* Content */}
      <div className="p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] space-y-6">
        {/* Sort By */}
        <div className="space-y-3">
          <h3 className="text-sm font-medium text-foreground">Sort By</h3>
          <Select value={filters.sortBy} onValueChange={(value) => updateFilters({ sortBy: value })}>
            <SelectTrigger className="h-12 bg-card/60 border-border/50">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
            ...
            <div className="grid grid-cols-2 gap-2">
              {GENRES.map((genre) => (
                <Button
                  ...
                  className={cn(
                    "h-12 text-sm transition-all duration-200 active:scale-95",
        ...
        <div className="space-y-3">
          ...
          {expandedSections.year && (
            <div className="px-3 py-4 bg-card/40 rounded-xl">
              <Slider
