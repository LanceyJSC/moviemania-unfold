-- Add comprehensive cinema data for better coverage only
...
-- Fix user_preferences table to use ON CONFLICT for upsert
CREATE OR REPLACE FUNCTION upsert_user_preferences(p_user_id uuid, p_updates jsonb)
...
BEGIN
    INSERT INTO user_preferences (user_id, location_latitude, location_longitude, location_city, location_country, notifications_enabled, email_notifications, push_notifications)
    VALUES (
        p_user_id,
        (p_updates->>'location_latitude')::numeric,
        (p_updates->>'location_longitude')::numeric,
        p_updates->>'location_city',
        p_updates->>'location_country',
        COALESCE((p_updates->>'notifications_enabled')::boolean, true),
        COALESCE((p_updates->>'email_notifications')::boolean, false),
        COALESCE((p_updates->>'push_notifications')::boolean, true)
    )
    ON CONFLICT (user_id) 
    DO UPDATE SET
        location_latitude = COALESCE((p_updates->>'location_latitude')::numeric, user_preferences.location_latitude),
        location_longitude = COALESCE((p_updates->>'location_longitude')::numeric, user_preferences.location_longitude),
        location_city = COALESCE(p_updates->>'location_city', user_preferences.location_city),
        location_country = COALESCE(p_updates->>'location_country', user_preferences.location_country),
        notifications_enabled = COALESCE((p_updates->>'notifications_enabled')::boolean, user_preferences.notifications_enabled),
        email_notifications = COALESCE((p_updates->>'email_notifications')::boolean, user_preferences.email_notifications),
        push_notifications = COALESCE((p_updates->>'push_notifications')::boolean, user_preferences.push_notifications),
        updated_at = now()
    RETURNING *;
    
    GET DIAGNOSTICS result = result;
    RETURN result;
END;
$$;