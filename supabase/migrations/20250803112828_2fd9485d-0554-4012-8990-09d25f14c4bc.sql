...
DROP INDEX IF EXISTS user_preferences_user_id_key;
...
-- Add more showtimes for the new cinemas
INSERT INTO cinema_showtimes (cinema_id, movie_id, movie_title, showtime, ticket_price, booking_url) 
SELECT 
    c.id,
    617126,
    'The Avengers',
    (NOW() + INTERVAL '1 hour' + (INTERVAL '3 hours' * generate_series(0, 4))),
    16.50,
    'https://example.com/book'
FROM cinemas c
WHERE c.name LIKE '%AMC%' AND c.city = 'Chicago'
LIMIT 1;
...
SELECT 
    ...
    1061474,
    'Wicked',
    (NOW() + INTERVAL '2 hours' + (INTERVAL '4 hours' * generate_series(0, 3))),
    18.50,
    'https://example.com/book'
FROM cinemas c
WHERE c.name LIKE '%Regal%' AND c.city = 'Atlanta'
LIMIT 1;